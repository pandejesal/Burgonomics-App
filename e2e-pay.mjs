...
const product = productSnap.empty
  ? null
  : { id: productSnap.docs[0].id, ...productSnap.docs[0].data() };

console.log(
  `product: ${product ? product.id + " " + (product.name || "") + " @" + (product.price || product.min_price) : "NONE FOUND"}`,
);
...
if (product) {
  ...
  console.log(
    `createPaymentOrder SUCCESS: ${r1.status} orderId=${r1.json?.orderId} keyId=${r1.json?.keyId} amount=${r1.json?.amount}`,
  );
  ...
  const realSig = crypto
    .createHmac("sha256", "kBSyl3K6BHH14CNAbylEzp57")
    .update(`${r1.json.orderId}|pay_TPgBIeczc3jw83`)
    .digest("hex");
  const r5 = await post(
    "/verifyPayment",
    {
