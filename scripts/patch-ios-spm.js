...
if (fs.existsSync(spmFile)) {
  ...
  // Pin capacitor-swift-pm to 8.4.2
  content = content.replace(/exact:\s*"8\.[0-9]+\.[0-9]+"/g, 'exact: "8.4.2"');
  content = content.replace(/from:\s*"8\.[0-9]+\.[0-9]+"/g, 'exact: "8.4.2"');
...
// 2. Patch all capacitor plugins in node_modules to pin capacitor-swift-pm to 8.4.2
...
for (const baseDir of nodeModulesDirs) {
  ...
  for (const pkg of packages) {
    ...
    if (fs.existsSync(pkgSwift)) {
      ...
      c = c.replace(/url:\s*"https:\/\/github\.com\/ionic-team\/capacitor-swift-pm\.git",\s*from:\s*"[^"]+"/g, 'url: "https://github.com/ionic-team/capacitor-swift-pm.git", exact: "8.4.2"');
      c = c.replace(/url:\s*"https:\/\/github\.com\/ionic-team\/capacitor-swift-pm\.git",\s*exact:\s*"8\.[0-9]+\.[0-9]+"/g, 'url: "https://github.com/ionic-team/capacitor-swift-pm.git", exact: "8.4.2"');
      fs.writeFileSync(pkgSwift, c, 'utf8');
      console.log(`✅ Pinned capacitor-swift-pm 8.4.2 in ${pkgSwift}`);
    }
  }
}
