...
exports.storeStatus = exports.pushMenu = void 0;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
const crypto = require("crypto");
const db = admin.firestore();
async function logWebhook(type, status, payload, execTimeMs, storeId = "unknown", storeName = "Unknown Store") {
    try {
 ...
 */
function isAuthorizedPetpoojaWebhook(req) {
    var _a;
    const secret = process.env.PETPOOJA_WEBHOOK_SECRET || ((_a = functions.config().petpooja) === null || _a === void 0 ? void 0 : _a.webhook_secret);
    if (!secret) {
 ...
 */
exports.pushMenu = functions.https.onRequest(async (req, res) => {
    var _a;
    if (req.method !== "POST") {
    ...
    try {
        ...
        for (const rest of restaurants) {
            const restId = (_a = rest.details) === null || _a === void 0 ? void 0 : _a.restID;
            if (!restId)
                continue;
            ...
            for (const cat of categories) {
                ...
                for (const item of items) {
                    ...
                    const customizations = (item.addon_groups || []).map((ag) => {
                        var _a;
                        return ({
                            id: (_a = ag.addon_group_id) === null || _a === void 0 ? void 0 : _a.toString(),
                            name: ag.addon_group_name,
                            minSelections: parseInt(ag.min_select, 10) || 0,
                            maxSelections: parseInt(ag.max_select, 10) || 1,
                            options: (ag.addon_items || []).map((ai) => {
                                var _a;
                                return ({
                                    id: (_a = ai.addon_item_id) === null || _a === void 0 ? void 0 : _a.toString(),
 ...
 */
exports.storeStatus = functions.https.onRequest(async (req, res) => {
    var _a;
    if (req.method !== "POST") {
        res.status(405).send("Method Not Allowed");
        return;
    ...
    try {
        ...
        if (!snapshot.empty) {
            ...
            storeName = ((_a = storeDoc.data()) === null || _a === void 0 ? void 0 : _a.name) || "Unknown Store";
